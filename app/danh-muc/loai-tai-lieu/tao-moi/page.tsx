...
export default function TaoMoiLoaiTaiLieuPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      // Kiểm tra tên loại tài liệu đã tồn tại chưa
      ...
      toast({
        title: "Thành công",
        description: "Đã thêm loại tài liệu mới thành công",
      })

      router.push('/danh-muc/loai-tai-lieu')
    } catch (error) {
      console.error('Lỗi khi thêm loại tài liệu:', error)
      toast({
        title: "Lỗi",
        description: "Không thể thêm loại tài liệu mới",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Thêm Loại tài liệu mới</h1>
          <p className="text-gray-600 mt-1">Nhập thông tin để tạo loại tài liệu mới</p>
        </div>
      </div>
